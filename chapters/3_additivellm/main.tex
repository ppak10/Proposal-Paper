\chapter{
    AdditiveLLM: Large Language Models Predict Defects in Metals Additive
    Manufacturing
}
\label{chapter:additivellm}
...
given prompt \cite{touvron_llama_2023, minaee_large_2024, devlin_bert_2019,
brown_language_2020, liu_roberta_2019}. LLMs are able to understand textual
descriptions of experimental results and respective context which enables its
application as a more human accessible research tool in a wide domain of fields
ranging from chemical engineering to robotics \cite{jadhav_llm-3d_2025,
ock_catalyst_2023, car_plato_2024, lorsung_explain_2024, shah_peptide-gpt_2024,
farimani_large_2024, chandrasekhar_amgpt_2024, ock_multimodal_2024,
balaji_gpt-molberta_2023, ock_adsorb-agent_2026, jadhav_large_2026}.
Implementations such as LLAMA \cite{touvron_llama_2023, grattafiori_llama_2024},
GPT \cite{brown_language_2020}, T5 \cite{raffel_exploring_2023} and BERT
\cite{devlin_bert_2019} provide a foundation for general purpose use where it
can then be fine-tuned for a specific downstream application.

The Bidirectional Encoder Representations from Transformers (BERT)
\cite{devlin_bert_2019} model is one way to architect a LLM and has garnered
much attention creating the base for extensions such as RoBERTa
(\textbf{R}obustly \textbf{o}ptimized \textbf{BERT} Approach)
\cite{liu_roberta_2019}, ALBERT (\textbf{A} \textbf{L}ite \textbf{BERT})
\cite{lan_albert_2020}, and DistilBERT \cite{sanh_distilbert_2020}. These models
which improves upon the existing implementation by either employing specific
training strategies such as a dataset, batch sizes, sequence lengths, and
hyperparameter optimization \cite{liu_roberta_2019, lan_albert_2020,
...
\begin{figure}[ht]
  ...
  \label{fig:additivellm_main}
\end{figure}

In this chapter a comparison of large language models fine-tuned on experimental
and simulation based LPBF melt pool data that able to predict potential defects
given a combination of process parameters is explored, reffered to as
AdditiveLLM (Figure \ref{fig:additivellm_main}). This work focuses upon
fine-tuning several models including: DistilBERT \cite{sanh_distilbert_2020},
SciBERT \cite{beltagy_scibert_2019}, LLama 3.2 \cite{grattafiori_llama_2024},
and T5 \cite{raffel_exploring_2023}. The selection of models utilize the smaller
parameter set variants as this best suits our need for an efficient and
lightweight model for specific inferences regarding defect regimes in LPBF
Additive Manufacturing. This approach infers upon the corpus of LPBF melt pool
...
Given an input text sequence outlining a LPBF build process with descriptions of
process parameters such as material, power, and velocity; AdditiveLLM aims to
...
of order dependent process parameters delineated with \texttt{\secsplit{[SEP]}}
separation tokens or queries regarding process parameters in the format of
natural language respectively. The \textit{Prompt} dataset aims to capture the
behavior of the unstructured input expected from a user while the
\textit{Baseline} dataset aims to definitively capture each process parameter.
These distinct approaches allow for a more flexible use case for the models as
the user is able to specifically tune inputs specific to their use case.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{chapters/3_additivellm/baseline_template.png}
  \caption{
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{chapters/3_additivellm/prompt_template.png}
  \caption{
...
include \texttt{\secsplit{material}}, \texttt{\secsplit{power}},
\texttt{\secsplit{velocity}}, \texttt{\secsplit{beam\_diameter}},
\texttt{\secsplit{hatch\_spacing}}, and \texttt{\secsplit{layer\_height}} and
...
\begin{figure}[ht]
  ...
  \label{fig:additivellm_dataset}
\end{figure}

A number of sources were used for our dataset which include literature and
simulation data points curated by \textit{Akbari et al.} in \textit{MeltpoolNet}
\cite{akbari_meltpoolnet_2022} and performed simulations obtained with FLOW-3D
\cite{flow_science_inc_flow-3d_2026}. (Figure \ref{fig:additivellm_dataset})
This amounts to a total of 2,779 collected data points spanning over 20
different materials, primarily consisting of Ti-6Al-4V and Stainless Steel 316L
(Figure \ref{fig:source_distributions}). With these sources, the dataset is
comprised of over 20 different materials varying with powers and velocity values
ranging from 25 W to 5000 W and 1 mm/s to 2000 mm/s respectively.
...
such as \textit{MeltpoolNet} \cite{akbari_meltpoolnet_2022} and a number of
FLOW-3D \cite{flow_science_inc_flow-3d_2026} simulations
(\texttt{\seqsplit{dimensions\_ti64.pkl}} and
\texttt{\seqsplit{dimensions\_ss316l\_10\_micron.csv}}) performed for this work.
The \texttt{\seqsplit{meltpoolclassification.csv}} from \textit{MeltpoolNet}
\cite{akbari_meltpoolnet_2022} provides the direct defect classification of a
combination of process parameters, however in cases where only the melt pool
dimensions are provided, the defect classifications are obtained using their
respective criterions. This data was further augmented by analyzing the
\textit{Lack of Fusion} defect criterion on 20 different layer heights and hatch
spacings ranging from 0 \textmu m to 950 \textmu m with the assumption that the
melt pool's width and depth remain the same. Each of these individual files, all
of the entries are shuffled and split to 75\%, 15\%, and 10\% for the train,
test, and validation datasets respectively. This allows for an equal
distribution of samples within each source in the overall dataset splits used to
fine-tune the LLM.
...
parameters of \texttt{\secsplit{material}}, \texttt{\secsplit{power}},
\texttt{\secsplit{velocity}}, \texttt{\secsplit{beam\_diameter}},
\texttt{\secsplit{hatch\_spacing}}, and \texttt{\secsplit{layer\_height}}
...
parameters to generate a prediction. This amounted to a total of 724,764
distinct input-label pair combinations with the 75\%, 15\%, 10\% train, test,
validation split including 543,573, 108,715, and 72,476 respectively.
...
SciBERT is a specialized implementation of the 110 million parameter BERT model
\cite{devlin_bert_2019} pretrained on large-scale scientific data including
...
Llama 3 is the latest iteration of LLMs developed by the AI team at Meta
\cite{grattafiori_llama_2024}. In comparison to the previous implementations,
Llama 3 is trained on a much larger dataset of 15 trillion tokens, boasts a
larger context window of up to 128K tokens, and consists of 405 billion
parameters for its largest model \cite{grattafiori_llama_2024}. For our
implementations we utilize Llama-3.2-1B, a 1 billion parameter implementation of
the the Llama 3 model that incorporates the logits of the 8 billion and 70
billion parameter Llama 3.1 models during pretraining in order to retain a
majority of their performance while reducing the hardware requirements for
finetuning \cite{grattafiori_llama_2024}. This model was fine-tuned with a batch
size of 512 and a learning rate of 2E-5.
...
experiment we utilize T5-Small \cite{raffel_exploring_2023} consisting of 60
million parameters for its efficient training speed and performance on a
specialized task such as defect classification \cite{raffel_exploring_2023}.
This model was fine-tuned with a batch size of 512 and a learning rate of 1E-4
following the guidelines outlined in the documentation
\cite{raffel_exploring_2023}.

\subsection{Fine-Tuning}
Each of the LLM models were attached with a classification head consisting of a
simple Multi-Layer Perceptron (MLP) layer producing four distinct binary
classifications and fine-tuned using the \textit{Baseline} and the
\textit{Prompt} dataset. In each case the weights of the LLM model remain
unfrozen and were trained along with the classification head for up to 25 epochs
for \textit{Baseline} fine-tuning and 2 epochs for \textit{Prompt} fine-tuning.
Models fine-tuned on the \textit{Prompt} dataset were done so for a shorter
number of epochs due to the lengthy training duration resulting from the dataset
being 100x larger than that of the \textit{Baseline}. Binary cross entropy was
used as the loss function for this multi-label classification task and the
inputs were tokenized with each LLM's respective tokenizer used in pretraining.
...
implementations we reference the findings by \textit{Akbari et al.}
\cite{akbari_meltpoolnet_2022} where the authors achieve a 85.78\%
...
Improvement in evaluation accuracy with epochs is most prominently displayed in
the Llama model where the extended training cycles drastically improved the
model's prediction accuracy, achieving an accuracy of 94.23\%, the highest
accuracy of all the trained models (Table \ref{tab:model_accuracy_values}). The
model achieves an F1 score of 0.9059 indicating a small amount of its achieved
accuracy is in part due to the composition of the dataset (Figure
\ref{fig:f1_per_class} bottom left). Llama's higher accuracy is expected as its
...
train as well. Notably, other models displayed plateauing or decreasing
validation accuracy after 20 epochs indicating that further fine-tuning may not
present additional benefit.

\begin{table}[h!]
\centering
\begin{tabular}{|c|c|c|c|c|c|}
\hline
\textbf{LLM}    & \textbf{5 Ep.} & \textbf{10 Ep.}    & \textbf{15 Ep.}    & \textbf{20 Ep.}    & \textbf{25 Ep.}   \\ \hline
...
\caption{
    ...
    25 epochs at 5 epoch increments.
}
...
\cite{golkar_xval_2024} and works by Thawani et al.
\cite{thawani_representing_2021} and Siyu Yu et al. \cite{yu_unlocking_2024}
have demonstrated success when processing numerically dense datasets and
generating predictions on out-of-distribution inputs. Thus, using a tokenizer
specialized in parsing numerical inputs would potentially help the model
understand the relationship between process parameters and generate predictions
...
with a wider range of materials, parameter sets, and additive processes (Chapter
\ref{chapter:dapt_am}). Longer fine-tuning of LLMs with the \textit{Prompt}
dataset could help expose the models to diverse input set of natural language
and produce predictions with greater accuracy. In addition, parts of the
existing models such as the tokenizer could be replaced with one better equipped
in understanding numerical inputs.

\clearpage

\section{Appendix}
\subsection{Model Training Resources and Runtimes}
\begin{table}[h]
    \centering
    \begin{tabular}{l l c c}
        \hline
        \textbf{Model}  & \textbf{\textit{Nvidia} GPU(s)} & \textbf{Epochs} & \textbf{Runtime (s)} \\
        \hline
        \textit{DistilBERT} & & & \\
        Baseline & RTX 2080 Ti 11 GB & 5 & 8886.8791 \\
        Baseline & RTX 2080 Ti 11 GB & 10 & 17381.732 \\
        Baseline & RTX 2080 Ti 11 GB & 20 & 34532.80 \\
        Baseline & A6000 48 GB & 25 & 44721.41 \\
        Prompt & RTX 2080 Ti 11 GB & 2 & 261917.09 \\
        \hline
        \textit{SciBERT} & & & \\
        Baseline & RTX 2080 Ti 11 GB & 5 & 29574.32 \\
        Baseline & RTX 2080 Ti 11 GB & 10 & 64870.11 \\
        Baseline & RTX 2080 Ti 11 GB & 15 & 48722.28 \\
        Baseline & A6000 48 GB & 20 & 36199.01 \\
        Baseline & A6000 48 GB & 25 & 45431.34 \\
        Prompt & A6000 48 GB & 2 & 302130.9155 \\
        \hline
        \textit{T5} & & & \\
        Baseline & RTX 2080 Ti 11 GB & 5 & 6616.4776 \\
        Baseline & RTX 2080 Ti 11 GB & 10 & 12668.9228 \\
        Baseline & RTX 2080 Ti 11 GB & 15 & 18136.53 \\
        Baseline & RTX 2080 Ti 11 GB & 20 & 25243.55 \\
        Baseline & RTX 2080 Ti 11 GB & 25 & 28502.06 \\
        Prompt & A6000 48 GB & 2 & 140927.38 \\
        \hline
        \textit{Llama} & & & \\
        Baseline & A6000 48 GB & 5 & 100295.01 \\
        Baseline & A6000 48 GB & 10 & 203343.7214 \\
        Baseline & 2 x A800 40 GB & 15 & 187949.0901 \\
        Baseline & A800 40 GB & 20 & 123264.0941 \\
        Baseline & A6000 48 GB & 25 & 151826.5571 \\
        Prompt & 2 x A6000 48 GB & 0.5 & 23913.9942 \\
        Prompt & 4 x Quadro 6000 24 GB & 2 & 24081.6544 \\
    \end{tabular}
    \caption{
        Hardware configurations used for fine-tuning the various large language
        models along with their corresponding epochs and runtimes.
    }
    \label{tab:model_training_configurations}
\end{table}
\clearpage

\subsection{Model Evaluation Runtimes}
\begin{table}[h]
    \centering
    \begin{tabular}{l c c}
        \hline
        \textbf{Model}  & \textbf{\textit{Baseline} (25 Epochs)} & \textbf{\textit{Prompt} (2 Epochs)} \\
        \hline
        DistilBERT & 1.295249 seconds & 1.539689 seconds\\
        SciBERT & 4.968237 seconds & 4.791886 seconds \\
        T5 & 0.882309 seconds & 0.180063 seconds \\
        Llama & 3.231006 seconds & 3.161664 seconds \\
    \end{tabular}
    \caption{Sample inference times obtained from querying fine-tuned LLMs hosted on CPU.}
    \label{tab:model_evaluation_runtimes}
\end{table}
\clearpage

\subsection{Prediction Distribution of LLM fine-tuned on \textit{Baseline} Dataset}
\begin{figure}[h]
    \centering
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{chapters/3_additivellm/DistilBERT_baseline_classification.pdf}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{chapters/3_additivellm/SciBERT_baseline_classifications.pdf}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{chapters/3_additivellm/Llama_baseline_classifications.pdf}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{chapters/3_additivellm/T5_baseline_classification.pdf}
    \end{subfigure}
    \caption{
        LLMs fine-tuned on \textit{Baseline} dataset have prediction
        distributions similar to that of the \textit{Augmented} dataset with
        the exception on the T5 model.
    }
    \label{fig:baseline_prediction_distribution}
\end{figure}
\clearpage

\subsection{Prediction Distribution of LLM fine-tuned on \textit{Prompt} Dataset}
\begin{figure}[h]
    \centering
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{chapters/3_additivellm/DistilBERT_prompt_classification.pdf}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{chapters/3_additivellm/SciBERT_prompt_classifications.pdf}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{chapters/3_additivellm/Llama_prompt_classifications.pdf}
    \end{subfigure}
    \begin{subfigure}{0.45\textwidth}
        \includegraphics[width=\linewidth]{chapters/3_additivellm/T5_prompt_classification.pdf}
    \end{subfigure}
    \caption{
        LLMs fine-tuned on \textit{Prompt} dataset exhibit a wider range of
        classification distribution labels with DistilBERT and SciBERT most
        closely resembling that of the \textit{Augmented} dataset.
    }
    \label{fig:prompt_prediction_distribution}
\end{figure}
\clearpage